...
int safe_input_int(const std::string& prompt) {
    ...
    while (true) {
        ...
        if (std::cin.fail()) {
            std::cout << "Ошибка! Пожалуйста, введите целое число в пределах int.\n";
            std::cin.clear();
...
int safe_input_positive_int(const std::string& prompt) {
    int value;
    while (true) {
        value = safe_input_int(prompt);
        if (value >= 0) {
            return value;
        }
        else {
...
int main()
{
    ...
    price = safe_input_positive_int("\nКакая стоимость оплаты: ");
    housing_office.Set_price(price);

    while (true) {
        housing_office.number_people = safe_input_positive_int("\nСколько сейчас людей в ней проживает: ");

        housing_office.number_paid = safe_input_positive_int("\nА сколько из них всё оплатили: ");
